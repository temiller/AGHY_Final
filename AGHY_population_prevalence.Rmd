...
eqm_prev%>%
  ...
  ggplot()+
  geom_histogram(aes(x=prevalence,fill=as.factor(year)),position="dodge",binwidth = 0.05)+
  facet_grid(water~.)
...
eqm_prev%>%
  ...
  mutate(year = as.integer(ifelse(year=="y1",2014,ifelse(year=="y2",2015,2016))))%>%
  ggplot()+
  geom_histogram(aes(x=prevalence,fill=water),position="dodge",binwidth = 0.05)+
