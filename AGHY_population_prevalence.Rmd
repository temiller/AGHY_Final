...
###################################################################
    ...
    for(i in 1:N.plots){
      ...
      logit(prev[i,1]) <- beta0.mean[water[i]] + ran.beta0.plot[plot[i]] + ran.beta0.year[year[1]] + (beta1.mean[water[i]] + ran.beta1.plot[plot[i]] + ran.beta1.year[year[1]]) * initial_prev[i]
...
AGHY_plot_dat <- AGHY_dat%>%group_by(newplot)%>%summarize(water = unique(water), initial_prev = unique(target_init_freq))%>%arrange(newplot)
