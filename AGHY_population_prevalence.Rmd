...
###################################################################
    ...
    for(i in 1:N.plots){
      ...
      logit(prev[i,1]) <- beta0.mean[water[i]] + ran.beta0.plot[plot[i]] + beta0.transeffect*trans_reduce[i] + (beta1.mean[water[i]] + ran.beta1.plot[plot[i]] + beta1.transeffect*trans_reduce[i]) * initial_prev[i]
      #   ran.beta0.year[year[1]] + 
      # + ran.beta1.year[year[1]]
      for(j in 2:N.years){
        logit(prev[i,j]) <- beta0.mean[water[i]] + ran.beta0.plot[plot[i]] +  (beta1.mean[water[i]] + ran.beta1.plot[plot[i]]) * prev[i,j-1]
      # + ran.beta1.year[year[j]] + 
      # ran.beta0.year[year[j]] +
      }
    }
    ...
    ##Eqm prevalence
    #for(i in 1:N.trt){
    #logit(eqm.prev[i])<-beta0.mean[i]/(1-beta1.mean[i])
    #}
...
AGHY_plot_dat <- AGHY_dat%>%group_by(newplot)%>%summarize(water = as.integer(unique(water)),transmission = unique(transmission), initial_prev = unique(target_init_freq))%>%mutate(trans_reduce = ifelse(transmission=="Reduce",1,0))%>%  arrange(newplot)
...
###################################################################
    ...
    for(i in 1:N.plots){
      ...
      #this is the 2013/14 transition, which is special b/c starting prev is known
      #I will need to include trans reduce effects here (not yet)
      logit(prev[i,1]) <- beta0.mean[water[i],year[1]] + ran.beta0.plot[plot[i]] + beta0.transeffect*trans_reduce[i] + (beta1.mean[water[i],year[1]] + ran.beta1.plot[plot[i]] + beta1.transeffect*trans_reduce[i]) * initial_prev[i]

      for(j in 2:N.years){
        logit(prev[i,j]) <- beta0.mean[water[i],year[j]] + ran.beta0.plot[plot[i]] +  (beta1.mean[water[i],year[j]] + ran.beta1.plot[plot[i]]) * prev[i,j-1]
...
ggplot(predictions)+
  geom_line(aes(x=x.prev,y=pred_prev,color=water))+
  facet_wrap(as.factor(year)~.)+
  geom_line(aes(x=x.prev,y=x.prev))
